...
impl ZeroPageWindow {
    pub fn new(sdl: &Sdl) -> Result<Self, String> {
        ...
        let view = Self {
            ...
            buffer: vec![0xFF; ZERO_PAGE_BYTES],
        };
    ...
    pub fn update(&mut self, bus: &Bus) -> Result<(), String> {
        ...
        for index in 0..ZERO_PAGE_BYTES {
            ...
            if self.buffer[index] != byte {
                ...
                self.draw_point(index as u32, byte)?;
                dirty = true;
            }
        }
    ...
    fn draw_point(&mut self, index: u32, byte: u8) -> Result<(), String> {
        let mut canvas = self.canvas.borrow_mut();
        let x = index % ZERO_PAGE_SIDE;
        let y = index / ZERO_PAGE_SIDE;
